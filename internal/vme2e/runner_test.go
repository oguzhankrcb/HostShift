...
func TestRunnerRendersLimaTemplatesAndSourceSafeManifests(t *testing.T) {
	...
	for _, expected := range []string{"/srv/hostshift-fixture/public/health", "getent passwd 501", "$3 >= 1000 && $3 < 60000", `MYSQL_SERVER_PACKAGE="default-mysql-server"`, "systemctl restart mysql || systemctl restart mariadb"} {
	...
	for _, expected := range []string{"nft add table inet hostshift", "nft list ruleset > /etc/nftables.conf"} {
	...
	if !containsString(stringSlice(commandList[8]), "plan") || !containsString(stringSlice(commandList[11]), "verify") {
		t.Fatalf("expected hostshift plan/verify commands: %#v", commandList)
	}
	if got := strings.Join(stringSlice(commandList[12]), " "); got != "limactl stop hostshift-ubuntu22-to-debian12-target" {
		t.Fatalf("unexpected target stop command: %s", got)
	}
	if got := strings.Join(stringSlice(commandList[13]), " "); got != "limactl start hostshift-ubuntu22-to-debian12-target" {
		t.Fatalf("unexpected target start command: %s", got)
	}
	if !containsString(stringSlice(commandList[16]), "verify") {
		t.Fatalf("expected post-reboot verify command: %#v", commandList[16])
	}
}
...
if [[ "${1:-}" == "sha256sum" ]]; then
  ...
  echo "def456  /etc/nginx/sites-available/hostshift-fixture.conf"
  exit 0
fi
